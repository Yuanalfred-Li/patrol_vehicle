...
ROUTE_FILE = Path("/tmp/test_tight_curve_actuator_lag_strong.yaml")
RECORD_FILE = Path("/tmp/test_tight_curve_actuator_lag_strong_recorded.yaml")
LOG_FILE = Path("/tmp/test_tight_curve_actuator_lag_strong_launch.log")
...
# 仿真压力参数，不代表已标定的实车数据。
STEERING_DELAY_SEC = 0.50
STEERING_RATE_LIMIT_PER_SEC = 300.0
...
try:
    ...
    max_abs_applied_steering = 0.0
    max_abs_route_applied_steering = 0.0
    route_saturation_steps = 0
    route_applied_saturation_steps = 0
    max_route_path_error = 0.0

    steering_delay_steps = max(
        1,
        int(round(STEERING_DELAY_SEC / DT)),
    )

    steering_history = deque(
        [0.0] * (steering_delay_steps + 1),
        maxlen=steering_delay_steps + 1,
    )

    applied_steering_request = 0.0
    ...
    while (
        ...
        if vehicle_command is not None:
            ...
            if abs(rpm) > 0.1:
                commanded_steering_request = max(
                ...
                steering_history.append(
                    commanded_steering_request
                )

                delayed_steering_request = (
                    steering_history[0]
                )

                maximum_change = (
                    STEERING_RATE_LIMIT_PER_SEC * DT
                )
                ...
                max_abs_applied_steering = max(
                    max_abs_applied_steering,
                    abs(applied_steering_request),
                )
                ...
                if (
                    ...
                    max_abs_route_applied_steering = max(
                        max_abs_route_applied_steering,
                        abs(applied_steering_request),
                    )
                    ...
                    if (
                        ...
                        route_applied_saturation_steps += 1
    ...
    applied_saturation_time = (
        route_applied_saturation_steps * DT
    )
    ...
    print(
        "MAX_ABS_APPLIED_STEERING:",
        round(max_abs_applied_steering, 3),
    )
    print(
        "MAX_ABS_ROUTE_APPLIED_STEERING:",
        round(max_abs_route_applied_steering, 3),
    )
    print(
    ...
    print(
        "ROUTE_APPLIED_SATURATION_STEPS:",
        route_applied_saturation_steps,
    )
    print(
        "ROUTE_APPLIED_SATURATION_TIME:",
        round(applied_saturation_time, 3),
    )
    print(
        "MAX_ROUTE_PATH_ERROR:",
