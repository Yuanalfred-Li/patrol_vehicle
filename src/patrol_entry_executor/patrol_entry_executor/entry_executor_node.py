...
class PatrolEntryExecutor(Node):
    ...
    def control_timer(self) -> None:
        ...
        previous_target = self.entry_path.poses[
            self.target_index - 1
        ]

        segment_x = (
            current_target.position.x
            - previous_target.position.x
        )
        segment_y = (
            current_target.position.y
            - previous_target.position.y
        )
        segment_length_squared = (
            segment_x * segment_x
            + segment_y * segment_y
        ...
        if tracking_error > maximum_tracking_error:
            self.safety_stop(
                f'entry path lateral error too large: '
                f'{tracking_error:.2f}m > '
                f'{maximum_tracking_error:.2f}m; '
                f'segment={self.target_index - 1}'
                f'->{self.target_index}'
            )
            return
